window.beliebt = [
    {
        'image': '',
        'title': 'Beliebt'
    },
    {
        'meal': 'Cheeseburger',
        'description': '',
        'choice': '',
        'price': 6.50
    },
    {
        'meal': 'Pizza Margherita',
        'description': '',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 7.00
    },
    {
        'meal': 'Pizza Döner',
        'description': 'mit Drehspießfleisch und Zwiebeln',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.50
    }
]

window.salate = [
    {
        'image': 'img/salad.jpg',
        'title': 'Salate'
    },
    {
        'meal': 'Gemischter Salat',
        'description': '',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 8.50
    },
    {
        'meal': 'Krautsalat',
        'description': 'mit Zwiebeln, Peperoni und Sauce, ohne gemischtem Salat',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 8.00
    },
    {
        'meal': 'Thunfisch Salat',
        'description': 'mit Ei',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 9.50
    },
    {
        'meal': 'Chef Salat',
        'description': 'mit Käse, Formvorderschinken, Thunfisch, Ei und Oliven',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 11.00
    },
    {
        'meal': 'Hawaii Salat',
        'description': 'mit Formvorderschinken, Ananas und Käse',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 9.50
    },
    {
        'meal': 'Döner Salat',
        'description': 'dazu Pizzabrot',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 11.50
    }
]

window.pizza = [
    {
        'image': 'img/pizza.jpg',
        'title': 'Pizza'
    },
    {
        'meal': 'Pizza Margherita',
        'description': '',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 7.00
    },
    {
        'meal': 'Pizza Salami',
        'description': 'mit Salami',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.00
    },
    {
        'meal': 'Pizza Schinken',
        'description': 'mit Fomvorderschinken',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.00
    },
    {
        'meal': 'Pizza Pepperoniwurst',
        'description': 'mit Pepperoniwurst',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.00
    },
    {
        'meal': 'Pizza Funghi',
        'description': 'mit Champignons',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.50
    },
    {
        'meal': 'Pizza Tonno',
        'description': 'mit Thunfisch und Zwiebeln',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.50
    },
    {
        'meal': 'Pizza Döner',
        'description': 'mit Drehspießfleisch und Zwiebeln',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 8.50
    },
    {
        'meal': 'Pizza 4 Jahreszeiten',
        'description': 'mit Formvorderschinken, Pilzen, Salami und Paprika',
        'choice': 'Wahl aus: Ø 28cm oder Ø 32cm',
        'price': 9.00
    }
]

window.calzone = [
    {
        'image': 'img/calzone.jpg',
        'title': 'Calzone'
    },
    {
        'meal': 'Calzone Napoli',
        'description': 'mit Formvorderschinken, Salami und Pilzen',
        'choice': 'Wahl aus: mit Ananas, mit Broccoli, mit Champignons, mit Drehspießfleisch, mit Ei und mehr.',
        'price': 9.00
    },
    {
        'meal': 'Calzone Tonno',
        'description': 'mit Thunfisch',
        'choice': 'Wahl aus: mit Ananas, mit Broccoli, mit Champignons, mit Drehspießfleisch, mit Ei und mehr.',
        'price': 9.50
    },
    {
        'meal': 'Calzone Hawaii',
        'description': 'mit Formvorderschinken und Ananas',
        'choice': 'Wahl aus: mit Ananas, mit Broccoli, mit Champignons, mit Drehspießfleisch, mit Ei und mehr.',
        'price': 9.50
    },
    {
        'meal': 'Calzone Frutti di Mare',
        'description': 'mit Meeresfrüchten',
        'choice': 'Wahl aus: mit Ananas, mit Broccoli, mit Champignons, mit Drehspießfleisch, mit Ei und mehr.',
        'price': 11.00
    },
    {
        'meal': 'Calzone Döner',
        'description': 'mit Dönerfleisch',
        'choice': 'Wahl aus: mit Ananas, mit Broccoli, mit Champignons, mit Drehspießfleisch, mit Ei und mehr.',
        'price': 9.50
    }
]

window.drehspieß = [
    {
        'image': 'img/drehspieß.jpg',
        'title': 'Drehspießgerichte'
    },
    {
        'meal': 'Jumbo Döner',
        'description': 'mit gemischtem Salat, Krautsalat und Zwiebeln',
        'choice': 'Wahl aus: mit Salatsauce, mit Tzatziki, ohne Sauce, ohne Eisbergsalat, ohne Gurken und mehr.',
        'price': 9.00
    },
    {
        'meal': 'Döner Fladenbrot',
        'description': 'mit gemischtem Salat, Krautsalat und Zwiebeln',
        'choice': 'Wahl aus: mit Salatsauce, mit Tzatziki, ohne Sauce, ohne Eisbergsalat, ohne Gurken und mehr.',
        'price': 7.00
    },
    {
        'meal': 'Döner Hawaii',
        'description': 'mit Ananas, Gouda und Tzatziki',
        'choice': 'Wahl aus: mit Salatsauce, mit Tzatziki, ohne Sauce, ohne Eisbergsalat, ohne Gurken und mehr.',
        'price': 8.00
    },
    {
        'meal': 'Vegetarischer Döner',
        'description': 'mit Hirtenkäse und Salat',
        'choice': 'Wahl aus: mit Salatsauce, mit Tzatziki, ohne Sauce, ohne Eisbergsalat, ohne Gurken und mehr.',
        'price': 6.50
    },
    {
        'meal': 'Döner Premium',
        'description': 'mit Drehspießfleisch und Sauce Hollandaise und mit Käse überbacken',
        'choice': 'Wahl aus: mit Salatsauce, mit Tzatziki, ohne Sauce, ohne Eisbergsalat, ohne Gurken und mehr.',
        'price': 8.50
    },
    {
        'meal': 'Döner Spezial',
        'description': 'mit Drehspießfleisch, Pommes frites und Salat',
        'choice': 'Wahl aus: mit Salatsauce, mit Tzatziki, ohne Sauce, ohne Eisbergsalat, ohne Gurken und mehr.',
        'price': 8.50
    }
]

window.teller = [
    {
        'image': 'img/teller.jpg',
        'title': 'Tellergerichte'
    },
    {
        'meal': 'Dönerteller',
        'description': 'mit Salat und Tzatziki',
        'choice': 'Wahl aus: mit Dressing, italienisch, mit Salatsauce, ohne Dressing und mit Pizzabrot.',
        'price': 11.50
    },
    {
        'meal': 'Dönerteller Hollandaise',
        'description': 'mit Sauce Hollandaise, Salat und Tzatziki und mit Käse überbacken',
        'choice': '',
        'price': 12.50
    },
    {
        'meal': 'Dönerauflauf',
        'description': 'mit Nudeln und Sauce Hollandaise und mit Käse überbacken',
        'choice': 'Wahl aus: mit Pommes frites, mit Reis oder ohne Beilage.',
        'price': 10.50
    },
    {
        'meal': 'Iskender',
        'description': 'Drehspießfleisch auf gerösteten Brotwürfeln mit spezieller Tomatensauce, dazu Salat und Salatsauce',
        'choice': 'Wahl aus: mit Pommes frites, mit Reis oder ohne Beilage.',
        'price': 12.00
    },
    {
        'meal': 'Chicken Picks (12 Stück)',
        'description': 'mit Salat, ohne Drehspießfleisch',
        'choice': 'Wahl aus: mit Pommes frites, mit Reis oder ohne Beilage.',
        'price': 10.50
    }
]

window.pasta = [
    {
        'image': 'img/pasta.jpg',
        'title': 'Pasta'
    },
    {
        'meal': 'Gnocchi mit Tomatensoße',
        'description': '',
        'choice': '',
        'price': 8.50
    },
    {
        'meal': 'Gnocchi Chef',
        'description': 'Kartoffelnudeln mit Sahnesauce, Champignons, Broccoli und mit Käse überbacken, ohne Brot',
        'choice': '',
        'price': 9.50
    },
    {
        'meal': 'Gnocchi Gorgonzola',
        'description': 'Kartoffelnudeln mit Sahnesauce, Champignons und Broccoli, ohne Brot',
        'choice': '',
        'price': 10.00
    },
    {
        'meal': 'Spaghetti Napoli',
        'description': 'mit Tomatensoße',
        'choice': '',
        'price': 8.00
    },
    {
        'meal': 'Spaghetti Bolognese',
        'description': 'mit Hackfleischsoße',
        'choice': '',
        'price': 9.00
    },
    {
        'meal': 'Spaghetti Carbonara',
        'description': 'mit Formvorderschinken und Ei-Sahnesauce',
        'choice': '',
        'price': 9.50
    },
    {
        'meal': 'Spaghetti Frutti di Mare',
        'description': 'mit Meeresfrüchten',
        'choice': '',
        'price': 11.00
    }
]

window.baguettes = [
    {
        'image': 'img/baguette.jpg',
        'title': 'Baguettes'
    },
    {
        'meal': 'Baguette Salami',
        'description': 'mit Tomaten und Zwiebeln',
        'choice': '',
        'price': 9.00
    },
    {
        'meal': 'Baguette Schinken',
        'description': 'mit Tomaten, Formvorderschinken und Zwiebeln',
        'choice': '',
        'price': 9.00
    },
    {
        'meal': 'Baguette Thunfisch',
        'description': 'mit Tomaten und Zwiebeln',
        'choice': '',
        'price': 9.50
    },
    {
        'meal': 'Baguette Hawaii',
        'description': 'mit Tomaten, Formvorderschinken und Ananas',
        'choice': '',
        'price': 9.50
    },
    {
        'meal': 'Baguette Mix',
        'description': '',
        'choice': '',
        'price': 10.00
    },
    {
        'meal': 'Baguette Döner',
        'description': 'mit Tomaten und Zwiebeln',
        'choice': '',
        'price': 9.50
    }
]

window.schnitzel = [
    {
        'image': 'img/schnitzel.jpg',
        'title': 'Schnitzel'
    },
    {
        'meal': 'Schnitzel Wiener Art',
        'description': '',
        'choice': '',
        'price': 12.50
    },
    {
        'meal': 'Schnitzel Gorgonzola',
        'description': 'mit Ananas und Sahnesauce',
        'choice': '',
        'price': 13.50
    },
    {
        'meal': 'Schnitzel Curry',
        'description': 'mit Curry-Sahnesauce',
        'choice': '',
        'price': 13.50,
    },
    {
        'meal': 'Schnitzel Käse',
        'description': 'mit vier Käsesorten und Sahnesauce',
        'choice': '',
        'price': 14.00
    },
    {
        'meal': 'Jägerschnitzel',
        'description': 'mit Jägersauce und Pilzen',
        'choice': '',
        'price': 14.50
    },
    {
        'meal': 'Zigeunerschitzel',
        'description': 'mit Zigeunersauce, Pilzen, Paprika, Zwiebeln und Peperoni',
        'choice': '',
        'price': 15.00
    },
    {
        'meal': 'Hawaii Schnitzel',
        'description': 'mit Ananas und Marsala-Sahnesauce und mit Käse überbacken',
        'choice': '',
        'price': 15.00
    }
]

window.burger = [
    {
        'image': 'img/burger.jpg',
        'title': 'Burger'
    },
    {
        'meal': 'Hamburger',
        'description': '',
        'choice': '',
        'price': 6.00
    },
    {
        'meal': 'Doppel Hamburger',
        'description': '',
        'choice': '',
        'price': 7.50
    },
    {
        'meal': 'Cheeseburger',
        'description': '',
        'choice': '',
        'price': 6.50
    },
    {
        'meal': 'Doppel Cheeseburger',
        'description': '',
        'choice': '',
        'price': 8.50
    },
    {
        'meal': 'Chickenburger',
        'description': '',
        'choice': '',
        'price': 6.50
    },
    {
        'meal': 'Dönerburger',
        'description': '',
        'choice': '',
        'price': 6.50
    }
]

window.myShoppingcart = [
    {
        'ammount': [],
        'meal': [],
        'price': []
    }
]


let dishes = [beliebt, salate, pizza, calzone, drehspieß, teller, pasta, baguettes, schnitzel, burger];